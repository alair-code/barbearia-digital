(function(){
  ...
  if(seo.palavrasChave)setMeta('keywords',seo.palavrasChave);
  ...
  const schema={"@context":"https://schema.org","@type":"BarberShop","name":c.nome||'Barbearia',"description":descricao,"telephone":c.telefone||'',"email":c.email||'',"url":location.origin,"address":{"@type":"PostalAddress","streetAddress":c.endereco||'',"addressLocality":c.cidade||'',"addressRegion":c.estado||'BR',"addressCountry":c.pais||'BR'}};
  ...
  document.querySelectorAll('.revelar').forEach(el=>el.classList.add('revelar'));
